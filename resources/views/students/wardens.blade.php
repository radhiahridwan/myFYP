                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 20px;">
                    @foreach ($wardens as $warden)
                        <div class="warden-card"
                            ...
                            <img src="{{ $warden->profile_picture ? asset('storage/' . $warden->profile_picture) : asset('images/admin-avatar.png') }}"
                                alt="Warden Photo"
                                style="width:100px; height:100px; border-radius:50%; object-fit:cover; margin-bottom:10px; border: 3px solid #004AAD;">
                            <h4 style="color: #004AAD; margin:5px 0;">{{ $warden->name }}</h4>
                            <p style="font-size:14px; color:#555; margin:3px 0;">📧 {{ $warden->email }}</p>
                            <!-- Updated phone number - check if phone_number exists in users table -->
                            <p style="font-size:14px; color:#555; margin:3px 0;">📞
                                {{ $warden->phone_number ?? 'Not provided' }}</p>
                        </div>
                    @endforeach
