        ...
        <div id="main-content" style="flex-grow: 1; width: 100%; transition: margin-left 0.3s ease;">
            ...
            <div style="padding: 25px; max-width: 800px; margin: 0 auto;">
                ...
                <div style="background: white; padding: 30px; border-radius: 12px; box-shadow: 0 2px 6px rgba(0,0,0,0.1);">
                    ...
                    <form action="{{ route('forms.facility-report.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                                ...
                                @foreach (['kerusi' => 'Chair (Kerusi)', 'meja' => 'Table (Meja)', 'almari' => 'Wardrobe (Almari)', 'katil' => 'Bed (Katil)', 'tilam' => 'Mattress (Tilam)'] as $key => $label)
                                    <div
                                        ...
                                        <div style="display: flex; align-items: center; gap: 10px;">
                                            <input type="number" name="inventory[{{ $key }}]" min="0"
                                                max="10"
                        ...
                        <div style="margin-bottom: 30px; padding: 20px; background: #f8fbff; border-radius: 8px;">
                            ...
                            <div style="margin-bottom: 25px;">
                                ...
                                <div style="display: flex; flex-direction: column; gap: 15px;">
                                    @foreach (['lampu' => 'Light (Lampu)', 'kipas' => 'Fan (Kipas)', 'plug_socket' => 'Plug Socket'] as $key => $item)
                                        <div
                                            style="background: white; padding: 15px; border-radius: 6px; border: 1px solid #e0e0e0;">
                                            <label
                                                style="font-weight: 500; display: block; margin-bottom: 10px; color: #333;">{{ $item }}</label>
                                            <div style="display: flex; gap: 20px; flex-wrap: wrap;">
                                                @foreach (['good' => 'Good', 'damaged' => 'Damaged', 'missing' => 'Missing'] as $status => $label)
                                                    <label
                                                        style="display: flex; align-items: center; gap: 8px; font-size: 14px;">
                                                        <input type="radio" name="condition[{{ $key }}]"
                            ...
                            <div style="margin-bottom: 25px;">
                                ...
                                <div style="display: flex; flex-direction: column; gap: 15px;">
                                    @foreach (['toilet' => 'Toilet', 'shower' => 'Shower', 'sink' => 'Sink'] as $key => $item)
                                        <div
                                            style="background: white; padding: 15px; border-radius: 6px; border: 1px solid #e0e0e0;">
                                            <label
                                                style="font-weight: 500; display: block; margin-bottom: 10px; color: #333;">{{ $item }}</label>
                                            <div style="display: flex; gap: 15px; flex-wrap: wrap;">
                                                @foreach (['good' => 'Good', 'clogged' => 'Clogged', 'no_water' => 'No Water', 'leaking' => 'Leaking', 'blocked' => 'Blocked'] as $status => $label)
                                                    <label
                                                        style="display: flex; align-items: center; gap: 8px; font-size: 14px;">
                                                        <input type="radio" name="condition[{{ $key }}]"
                                                            value="{{ $status }}" required>
                            ...
                            <div>
                                ...
                                <div style="display: flex; flex-direction: column; gap: 15px;">
                                    @foreach (['langsir' => 'Curtain (Langsir)', 'pintu' => 'Door (Pintu)', 'tingkap' => 'Window (Tingkap)'] as $key => $item)
                                        <div
                                            style="background: white; padding: 15px; border-radius: 6px; border: 1px solid #e0e0e0;">
                                            <label
                                                style="font-weight: 500; display: block; margin-bottom: 10px; color: #333;">{{ $item }}</label>
                                            <div style="display: flex; gap: 15px; flex-wrap: wrap;">
                                                @if ($key == 'langsir')
                                                    @foreach (['good' => 'Good', 'torn' => 'Torn', 'missing' => 'Missing'] as $status => $label)
                                                        <label
                                                            style="display: flex; align-items: center; gap: 8px; font-size: 14px;">
                                                            <input type="radio" name="condition[{{ $key }}]"
                                                                value="{{ $status }}" required>
                                                    ...
                                                    @endforeach
                                                @else
                                                    @foreach (['good' => 'Good', 'stuck' => 'Stuck', 'broken_lock' => 'Broken Lock', 'broken' => 'Broken'] as $status => $label)
                                                        <label
                                                            style="display: flex; align-items: center; gap: 8px; font-size: 14px;">
                                                            <input type="radio" name="condition[{{ $key }}]"
                                                                value="{{ $status }}" required>
