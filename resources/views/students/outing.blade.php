            ...
            <div style="padding: 30px; max-width: 800px; margin: 0 auto;">
                ...
                <div
                    ...
                    <a href="/student/outing/form"
                        style="background: {{ $page == 'form' ? '#004AAD' : 'white' }}; padding: 20px; border-radius: 12px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); text-decoration: none; color: {{ $page == 'form' ? 'white' : '#333' }}; text-align: center; transition: transform 0.2s, box-shadow 0.2s;">
                    ...
                    <a href="/student/outing/current"
                        style="background: {{ $page == 'current' ? '#004AAD' : 'white' }}; padding: 20px; border-radius: 12px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); text-decoration: none; color: {{ $page == 'current' ? 'white' : '#333' }}; text-align: center; transition: transform 0.2s, box-shadow 0.2s;">
                        <div style="font-size: 24px; margin-bottom: 10px;"></div>
                    ...
                    <a href="/student/outing/history"
                        style="background: {{ $page == 'history' ? '#004AAD' : 'white' }}; padding: 20px; border-radius: 12px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); text-decoration: none; color: {{ $page == 'history' ? 'white' : '#333' }}; text-align: center; transition: transform 0.2s, box-shadow 0.2s;">
                        <div style="font-size: 24px; margin-bottom: 10px;"></div>
                ...
                <div
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px;">
                        <div>
                            ...
                            <p style="margin: 5px 0 0 0; color: #333;">{{ $student->name }}</p>
                        </div>
                        <div>
                            <label style="font-weight: bold; color: #555;">Student ID</label>
                            <p style="margin: 5px 0 0 0; color: #333;">{{ $student->student_id }}</p>
                        </div>
                        <div>
                            <label style="font-weight: bold; color: #555;">Room Number</label>
                            <p style="margin: 5px 0 0 0; color: #333;">{{ $student->hostel_room }}</p>
                        </div>
                    </div>
                </div>
                ...
                @if ($page == 'form')
                    <div
                        ...
                        <form action="{{ route('outing.store') }}" method="POST">
                            @csrf
                ...
                @if ($page == 'current')
                    <div
                        ...
                        @if ($outingsToDisplay->count() > 0)
                            @foreach ($outingsToDisplay as $outing)
                                <div
                                    ...
                                    <div
                                        ...
                                        <div>
                                            ...
                                            <p style="margin: 5px 0 0 0; color: #333;">
                                                {{ $outing->departure_time->format('M j, Y g:i A') }}
                                            </p>
                                        </div>
                                        <div>
                                            <label style="font-weight: bold; color: #555;">Expected Return</label>
                                            <p style="margin: 5px 0 0 0; color: #333;">
                                                {{ $outing->expected_return_time->format('M j, Y g:i A') }}
                                            </p>
                                        </div>
                                        <div>
                                            <label style="font-weight: bold; color: #555;">Destination</label>
                                            <p style="margin: 5px 0 0 0; color: #333;">{{ $outing->destination }}</p>
                                        </div>
                                    </div>
                                    <div style="margin-bottom: 20px;">
                                        <label style="font-weight: bold; color: #555;">Purpose</label>
                                        <p style="margin: 5px 0 0 0; color: #333;">{{ $outing->purpose }}</p>
                                    </div>
                                    <div
                                        ...
                                        <div>
                                            ...
                                            <p style="margin: 5px 0 0 0; color: #333;">
                                                {{ $outing->emergency_contact_number }}</p>
                                        </div>
                                        <div>
                                            <label style="font-weight: bold; color: #555;">Relationship</label>
                                            <p style="margin: 5px 0 0 0; color: #333;">
                                                {{ $outing->emergency_contact_relationship }}</p>
                                        </div>
                                    </div>

                                    <!-- ADDED DELETE BUTTONS HERE -->
                                    <div style="display: flex; gap: 10px; margin-top: 15px;">
                                        <form action="{{ route('outing.mark-returned', $outing->id) }}" method="POST">
                                            @csrf
                                            <button type="submit"
                                        ...
                                        <form action="{{ route('outing.delete', $outing->id) }}" method="POST"
                                        ...
                                        @foreach ($outings as $outing)
                                            <tr style="border-bottom: 1px solid #e0e0e0;">
                                                <td style="padding: 12px;">
                                                    {{ $outing->departure_time->format('M j, Y g:i A') }}</td>
                                                <td style="padding: 12px;">
                                                    @if ($outing->actual_return_time)
                                                        {{ $outing->actual_return_time->format('M j, Y g:i A') }}
                                                    @else
                                                        {{ $outing->expected_return_time->format('M j, Y g:i A') }}
                                            ...
                                            background: {{ $outing->status == 'out' ? '#fff3cd' : '#d4edda' }}; 
                                            color: {{ $outing->status == 'out' ? '#856404' : '#155724' }};">
                                                        {{ $outing->status == 'out' ? 'OUT' : 'RETURNED' }}
                                                    </span>
                                                </td>
                                            </tr>
                                        @endforeach
