        ...
        <div id="main-content" style="flex-grow:1; width:100%; transition:margin-left 0.3s ease;">
            ...
            <div style="padding:25px; width:100%; max-width:1200px; margin:auto;">
                ...
                <div
                    ...
                    <form id="ruleForm" method="POST" action="{{ route('admin.rules.store') }}"
                ...
                <div id="rulesList"
                    ...
                    <div id="rulesContainer">
                        @forelse($rules as $rule)
                            <div class="rule-card" id="rule-{{ $rule->id }}"
                                ...
                                <div id="content-{{ $rule->id }}"
                                    ...
                                    <div style="display:flex; gap:10px;">
                                        <button type="button" class="editBtn" data-id="{{ $rule->id }}"
                                            data-title="{{ $rule->title }}" data-description="{{ $rule->description }}"
                                            data-image="{{ $rule->image ? asset('storage/' . $rule->image) : '' }}"
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function createRuleCardHTML(rule) {
                const imageHtml = rule.image ?
                    `<img src="{{ asset('storage/') }}/${rule.image}" alt="Rule Image" style="width:120px; border-radius:8px; margin-bottom:10px;">` :
                    '';
        ...
        `;
            ...
            async function deleteRule(ruleId) {
                ...
                try {
                    const response = await fetch('{{ route('admin.rules.delete', '') }}/' + ruleId, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Accept': 'application/json',
            ...
            function resetForm() {
                ...
                document.getElementById('ruleForm').action = '{{ route('admin.rules.store') }}';
